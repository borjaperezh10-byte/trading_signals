...
# ============================================================================
#  DESCARGA DE DATOS  (requiere internet -- se ejecuta en GitHub Actions)
# ============================================================================

# Lista de respaldo del Nasdaq 100. Cambia poco (revision anual en diciembre),
# asi que tenerla fija evita depender de que Wikipedia este accesible.
NDX_RESPALDO = [
    "AAPL","ABNB","ADBE","ADI","ADP","ADSK","AEP","AMAT","AMD","AMGN","AMZN",
    "ANSS","APP","ARM","ASML","AVGO","AZN","BIIB","BKNG","BKR","CCEP","CDNS",
    "CDW","CEG","CHTR","CMCSA","COST","CPRT","CRWD","CSCO","CSGP","CSX","CTAS",
    "CTSH","DASH","DDOG","DXCM","EA","EXC","FANG","FAST","FTNT","GEHC","GFS",
    "GILD","GOOG","GOOGL","HON","IDXX","INTC","INTU","ISRG","KDP","KHC","KLAC",
    "LIN","LRCX","LULU","MAR","MCHP","MDB","MDLZ","MELI","META","MNST","MRVL",
    "MSFT","MU","NFLX","NVDA","NXPI","ODFL","ON","ORLY","PANW","PAYX","PCAR",
    "PDD","PEP","PLTR","PYPL","QCOM","REGN","ROP","ROST","SBUX","SMCI","SNPS",
    "TEAM","TMUS","TSLA","TTD","TTWO","TXN","VRSK","VRTX","WBD","WDAY","XEL","ZS",
]

CABECERAS = {
    "User-Agent": "Mozilla/5.0 (compatible; swing-signals/1.0; +https://github.com)"
}


def _sp500_desde_wikipedia():
    """Lee la tabla de componentes del S&P 500 identificandose correctamente."""
    import io
    import requests
    resp = requests.get(
    ...
    return pd.read_html(io.StringIO(resp.text))[0]["Symbol"].tolist()


def _sp500_desde_github():
    """Respaldo: dataset publico de componentes del S&P 500."""
    url = ("https://raw.githubusercontent.com/datasets/"
           "s-and-p-500-companies/main/data/constituents.csv")
    return pd.read_csv(url)["Symbol"].tolist()


def _ndx_desde_wikipedia():
    import io
    import requests
    ...
    for t in tablas:
        for col in ("Ticker", "Symbol"):
            if col in t.columns and len(t) > 80:
                return t[col].tolist()
    raise ValueError("Tabla del Nasdaq 100 no encontrada")


def obtener_universo():
    """Devuelve los tickers de S&P 500 + Nasdaq 100, con varios respaldos.

    Wikipedia bloquea peticiones sin User-Agent (error 403), asi que
    probamos en cascada hasta conseguir una lista valida.
    """
    sp500 = []
    for intento in (_sp500_desde_wikipedia, _sp500_desde_github):
        try:
            sp500 = intento()
            print(f"    S&P 500 via {intento.__name__}: {len(sp500)} valores")
            break
        except Exception as e:
            print(f"    {intento.__name__} fallo: {type(e).__name__} {e}")

    try:
        ndx = _ndx_desde_wikipedia()
        print(f"    Nasdaq 100 via Wikipedia: {len(ndx)} valores")
    except Exception as e:
        print(f"    Nasdaq 100 via Wikipedia fallo ({type(e).__name__}), uso lista fija")
        ndx = NDX_RESPALDO

    if not sp500:
        print("    S&P 500 no disponible, sigo solo con el Nasdaq 100")

    universo = sorted(set(
        str(t).strip().replace(".", "-") for t in list(sp500) + list(ndx)
        if isinstance(t, str) and t.strip()
    ))
    if len(universo) < 50:
        raise RuntimeError(f"Universo demasiado pequenyo ({len(universo)})")
    return universo
